...
///////////////////////////////////////////////////////////////////////
std::vector<std::vector<int>> prune_keypoints(std::vector<std::vector<int>> &keypoints, int min_dist_to_bbox, int x_dim, int y_dim, int z_dim) {

  // Filter out keypoints too close to the bounding box of the voxel volume
    std::vector<std::vector<int>> valid_keypoints;
    for (int i = 0; i < keypoints.size(); i++)
        if (keypoints[i][0] - min_dist_to_bbox >= 0 && keypoints[i][0] + min_dist_to_bbox < x_dim &&
            keypoints[i][1] - min_dist_to_bbox >= 0 && keypoints[i][1] + min_dist_to_bbox < y_dim &&
            keypoints[i][2] - min_dist_to_bbox >= 0 && keypoints[i][2] + min_dist_to_bbox < z_dim)
