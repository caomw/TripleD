...
std::vector<std::vector<float>> ddd_get_keypoint_feat(float* volume, int x_dim, int y_dim, int z_dim, std::vector<std::vector<int>> &keypoints, int patch_radius, bool is_verbose) {
  ...
  std::string model_filename = "model/dddnet.marvin";
  std::string architecture_filename = "model/featnet.json";
  std::string new_architecture_filename = "model/featnet_" + instance_id + ".json";
  std::string marvin_filename = "model/marvin";
...
void ddd_compare_feat(std::vector<std::vector<float>> &feat1, std::vector<std::vector<float>> &feat2, std::vector<std::vector<float>> &score_matrix, bool is_verbose) {
  ...
  std::string model_filename = "model/dddnet.marvin";
  std::string architecture_filename = "model/metricnet.json";
  std::string new_architecture_filename = "model/metricnet_" + instance_id + ".json";
  std::string marvin_filename = "model/marvin";
  std::string cuda_lib_dir = "/usr/local/cuda/lib64";
  ...
  model_filename = "model/dddnet.marvin";
...
void ddd_align_feature_cloud(std::vector<std::vector<float>> &world_keypoints1, std::vector<std::vector<float>> &feat1, std::vector<std::vector<float>> &score_matrix1,
    ...
    tic();
    inlier_pairs = ransacfitRt(world_keypoints1, world_keypoints2, match_idx, ransac_k, max_ransac_iter, ransac_thresh, Rt, ddd_verbose);
...
void align2tsdf(float* scene_tsdf1, int x_dim1, int y_dim1, int z_dim1, float world_origin1_x, float world_origin1_y, float world_origin1_z,
  ...
  ddd_align_feature_cloud(world_keypoints1, feat1, score_matrix1, world_keypoints2, feat2, score_matrix2, k_match_score_thresh, ransac_k, max_ransac_iter, ransac_thresh, Rt, inlier_pairs);
}